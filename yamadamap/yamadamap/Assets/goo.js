    ...
    function successCallback(position) {
        ...
        nttMap.addEventListener("click",
            function (e) {
                ...
                function createPop(data, index, ar) {
                    const post = coordinateNtt(data.lad, data.longi);

                    var pos = new NttPoint(post.y, post.x);
// 表示するアイコンやサイズなどの設定
                    var markerOptions = {
                        color: '#00bfff'
                    };
...
// マーカーオブジェクトを地図画面オブジェクト
                    nttMap.addGeometry(nttMarker, index);
...
//------------------------------------------------------------popup
                    ...
                    <p style="position:absolute;z-index:2;">` + data.soundid + `</p>
...
// 吹き出しオブジェクトを生成(この時点ではまだ地図上には表示されない)
                    ...
                    nttMap.addPopupWindow(nttPopupWindow, index);

                }


                dumys.forEach(createPop);
